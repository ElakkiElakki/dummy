...
public abstract class BasePage extends CommonPage {
    ...
    protected WebElement waitForFirstVisible(By... locators) {
        final WebElement[] found = new WebElement[1];
        boolean success = WaitUtils.waitUntil(new WaitUtils.Condition() {
            @Override
            public boolean evaluate() {
                try {
                    found[0] = findFirstVisible(locators);
                    return found[0] != null;
                } catch (Exception ignored) {
                    return false;
                }
            }
        }, ConfigReader.getInt("explicit.wait.seconds", 25), 500L);

        if (!success || found[0] == null) {
    ...
    protected boolean typeAndSelectFirstSuggestion(By inputLocator, String value) {
        ...
        return typeAndSelectFirstSuggestion(input, value);
    }

    protected boolean typeAndSelectFirstSuggestion(WebElement input, String value) {
        input.click();
        input.clear();
